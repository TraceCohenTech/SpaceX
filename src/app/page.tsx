...
const revenueData = [
  { year: "2020", starlink: 0.05, launch: 1.95 }, { year: "2021", starlink: 1.0, launch: 2.0 },
  { year: "2022", starlink: 2.5, launch: 2.1 }, { year: "2023", starlink: 5.5, launch: 3.2 },
  { year: "2024", starlink: 8.0, launch: 6.0 }, { year: "2025", starlink: 10.6, launch: 5.4 },
  { year: "2026E", starlink: 15.0, launch: 7.0 },
];

const valuationData = [
  { date: "2012", valuation: 2.4 }, { date: "2015", valuation: 12 },
  { date: "2019", valuation: 33 }, { date: "2020", valuation: 46 },
  { date: "2021", valuation: 100 }, { date: "2022", valuation: 137 },
...
const starlinkGrowth = [
  { year: "2020", satellites: 1020, subscribers: 0.01 }, { year: "2021", satellites: 2040, subscribers: 0.15 },
  { year: "2022", satellites: 3900, subscribers: 1.0 }, { year: "2023", satellites: 5700, subscribers: 2.3 },
  { year: "2024", satellites: 7500, subscribers: 4.0 }, { year: "2025", satellites: 8500, subscribers: 9.2 },
...
const ipoCompsData = [
  { name: "SpaceX", multiple: 94 }, { name: "Aramco", multiple: 18 },
  { name: "Rivian", multiple: 90 }, { name: "Palantir", multiple: 55 },
  { name: "RKLB", multiple: 28 },
];

const boosterReuse = [
  { booster: "B1062", flights: 23 }, { booster: "B1061", flights: 22 },
  { booster: "B1060", flights: 21 }, { booster: "B1058", flights: 20 },
  { booster: "B1069", flights: 19 }, { booster: "B1067", flights: 18 },
];
...
const milestones = [
  ...
  { year: "2024", event: "134 Launches", desc: "More launches than every other entity combined" },
  { year: "2026", event: "IPO", desc: "Targeting $1.5T — the largest IPO in history" },
];

const vehicleFleet = [
  { name: "Falcon 9", type: "Medium-Lift", height: "70m", payload: "22,800 kg", engines: "9 Merlin", reuses: "23x record", launches: "370+", color: "#3b82f6" },
  { name: "Falcon Heavy", type: "Heavy-Lift", height: "70m", payload: "63,800 kg", engines: "27 Merlin", reuses: "Side boosters", launches: "11", color: "#22d3ee" },
  ...
  { name: "Dragon", type: "Crew & Cargo", height: "8.1m", payload: "6,000 kg", engines: "SuperDraco", reuses: "Multi-flight", launches: "50+", color: "#10b981" },
];
    ...
    <main className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      <div className="border-y border-blue-500/10 bg-slate-950/60 backdrop-blur-sm">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-12 py-8 sm:py-10 grid grid-cols-3 sm:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-6">
          {[
            ...
            { value: "400+", label: "Orbital Launches", color: C.blue },
            { value: "9.2M", label: "Starlink Subs", color: C.green },
            { value: "$16B", label: "2025 Revenue", color: C.orange },
            { value: "98%", label: "Landing Rate", color: C.cyan },
          ].map((s, i) => (
      ...
      <Section title="IPO Overview" sub="SpaceX is preparing to file for the largest public offering in history, targeting a June 2026 listing">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {[
            { label: "Target Valuation", value: "$1.5 — $1.75T", desc: "Most valuable IPO ever, surpassing Saudi Aramco ($1.7T)", color: C.cyan },
            { label: "Capital Raise", value: "$50 — $75B", desc: "~3.3% equity float. Funding Starship, Starlink, and space data centers", color: C.blue },
            ...
            { label: "2025 Revenue", value: "$16.0B", desc: "Starlink drove $10.6B (67%) with 54% EBITDA margins", color: C.orange },
            { label: "2025 EBITDA", value: "$7.5B", desc: "Reflecting Starlink profitability and growing launch margins", color: C.yellow },
            { label: "Starlink Subs", value: "9.2M", desc: "Doubled two years running. On track to 18.4M in 2026", color: C.cyan },
          ].map((item, i) => (
      ...
      {/* ══════ LAUNCHES ══════ */}
      <Section title="Launch History" sub="From 1 failed launch in 2006 to 134 in 2024 — more than every other entity on Earth combined">
      ...
      {/* ══════ REVENUE ══════ */}
      <Section title="Revenue Trajectory" sub="Starlink now drives 67% of total revenue — the engine behind the IPO">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          <div className="card p-4 sm:p-6">
            <h3 className="text-xs tracking-[0.12em] text-slate-500 uppercase mb-5">Revenue Breakdown ($B)</h3>
          ...
          <div className="card p-4 sm:p-6">
            <h3 className="text-xs tracking-[0.12em] text-slate-500 uppercase mb-5">Valuation Growth ($B)</h3>
            <ResponsiveContainer width="100%" height={320}>
      ...
      <div className="relative">
        ...
        <div className="relative">
          <Section title="Starlink" sub="The world's largest satellite constellation — 9.2M subscribers across 150+ countries, doubling annually">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
              {[
                { val: "9.2M", label: "Subscribers", sub: "Doubling every year", color: C.green },
                { val: "$10.6B", label: "2025 Revenue", sub: "67% of SpaceX total", color: C.cyan },
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
              <div className="card bg-black/50 backdrop-blur-md p-4 sm:p-6">
                <h3 className="text-xs tracking-[0.12em] text-slate-500 uppercase mb-5">Constellation Size</h3>
              ...
              <div className="card bg-black/50 backdrop-blur-md p-4 sm:p-6">
                <h3 className="text-xs tracking-[0.12em] text-slate-500 uppercase mb-5">Subscriber Growth (Millions)</h3>
                <ResponsiveContainer width="100%" height={280}>
      ...
      {/* ══════ VEHICLE FLEET ══════ */}
      <Section title="Vehicle Fleet" sub="Four vehicles. One mission. Making life multiplanetary.">
      ...
      {/* ══════ BOOSTER REUSE ══════ */}
      <Section title="Reuse Leaderboard" sub="A single Falcon 9 booster has flown 23 times — the economics that make the IPO possible">
        <div className="card p-4 sm:p-6 md:p-8">
      ...
      {/* ══════ COST ══════ */}
      <Section title="Cost Revolution" sub="SpaceX slashed launch costs 10x. Starship aims for 100x.">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          <div className="card p-4 sm:p-6">
            <h3 className="text-xs tracking-[0.12em] text-slate-500 uppercase mb-5">Cost Per Launch ($M)</h3>
            <ResponsiveContainer width="100%" height={320}>
          ...
          <div className="card p-4 sm:p-6">
            <h3 className="text-xs tracking-[0.12em] text-slate-500 uppercase mb-5">Cost Per kg to LEO ($)</h3>
            <ResponsiveContainer width="100%" height={320}>
              <BarChart data={costComparison}>
                <CartesianGrid strokeDasharray="3 3" stroke={C.grid} />
                <XAxis dataKey="vehicle" stroke={C.dim} tick={{ fill: C.muted, fontSize: 10 }} />
      ...
      {/* ══════ COMPETITIVE ══════ */}
      <Section title="Competitive Landscape" sub="There is no close second">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
          <div className="card p-4 sm:p-6">
            <h3 className="text-xs tracking-[0.12em] text-slate-500 uppercase mb-5">Global Launch Market Share (%)</h3>
            <ResponsiveContainer width="100%" height={280}>
          ...
          <div className="card p-4 sm:p-6">
            <h3 className="text-xs tracking-[0.12em] text-slate-500 uppercase mb-5">Capability Index</h3>
            <ResponsiveContainer width="100%" height={280}>
        ...
        <div className="card overflow-x-auto">
          <table className="w-full sx-table min-w-[500px]">
            <thead><tr>
              {["Company", "Launches", "Revenue", "Valuation", "Reuse"].map((h) => <th key={h}>{h}</th>)}
            </tr></thead>
            <tbody>
      ...
      {/* ══════ GOV CONTRACTS ══════ */}
      <Section title="Government Contracts" sub="$18.6B in combined NASA, DoD, and Space Force contract value">
        <div className="card p-4 sm:p-6 md:p-8">
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={govContracts} layout="vertical" margin={{ left: 10 }}>
              ...
              <YAxis type="category" dataKey="name" stroke={C.dim} tick={{ fill: C.muted, fontSize: 10 }} width={100} />
              <Tooltip content={<Tip />} />
      ...
      {/* ══════ IPO COMPS ══════ */}
      <Section title="Valuation Comps" sub="At ~94x revenue, SpaceX would carry the richest multiple of any mega-IPO in history">
        <div className="card p-4 sm:p-6 md:p-8">
          <ResponsiveContainer width="100%" height={280}>
      ...
      {/* ══════ STARSHIP TESTS ══════ */}
      <Section title="Starship Flight Tests" sub="The most powerful rocket ever built — 33 engines, 16.7 million lbf of thrust">
        <div className="card overflow-x-auto">
          <table className="w-full sx-table min-w-[480px]">
            <thead><tr>{["Flight", "Date", "Result", "Milestone"].map((h) => <th key={h}>{h}</th>)}</tr></thead>
            <tbody>
              {[
                { f: "IFT-1", d: "Apr 2023", r: "FAILURE", c: C.red, detail: "FTS activated — vehicle tumbled" },
                { f: "IFT-2", d: "Nov 2023", r: "PARTIAL", c: C.yellow, detail: "First stage separation achieved" },
                { f: "IFT-3", d: "Mar 2024", r: "PARTIAL", c: C.yellow, detail: "Reached space, re-entry data" },
                { f: "IFT-4", d: "Jun 2024", r: "SUCCESS", c: C.green, detail: "Both stages survived" },
                { f: "IFT-5", d: "Oct 2024", r: "HISTORIC", c: C.cyan, detail: "Booster caught by Mechazilla" },
                { f: "IFT-6", d: "Nov 2024", r: "PARTIAL", c: C.yellow, detail: "Ship orbited, deployed test sats" },
                { f: "IFT-7", d: "Jan 2025", r: "SUCCESS", c: C.green, detail: "Orbit + booster catch again" },
              ].map((fl, i) => (
                <tr key={i}>
      ...
      {/* ══════ TIMELINE ══════ */}
      <Section title="The SpaceX Story" sub="22 years from startup to the largest IPO in history">
        <div className="relative">
      ...
      {/* ══════ RISK/REWARD ══════ */}
      <Section title="IPO Risk/Reward" sub="What investors need to know before the largest public offering in history">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {[
            { tag: "BULL", title: "Starlink Dominance", desc: "9.2M subs, 54% EBITDA margins, doubling annually. T-Mobile direct-to-cell expands TAM to billions.", color: C.green },
            { tag: "BULL", title: "Monopoly on Reuse", desc: "No competitor has demonstrated orbital reuse at scale. Falcon 9 margins widen every reflight.", color: C.green },
            { tag: "BULL", title: "Starship Unlocks Everything", desc: "Space data centers, Mars missions, point-to-point transport. $67/kg to LEO changes everything.", color: C.green },
            { tag: "BEAR", title: "Key-Person Risk", desc: "Musk leads two $1T+ companies simultaneously. \"Most divisive stock to join the market.\"", color: C.red },
            { tag: "BEAR", title: "94x Revenue Multiple", desc: "Richest multiple of any mega-IPO in history. Requires sustained hypergrowth to justify.", color: C.red },
            { tag: "BEAR", title: "Macro & Regulatory", desc: "Tariff threats, rate jitters, unfiled S-1, global roadshow crunch, spectrum regulation.", color: C.red },
          ].map((card, i) => (
      ...
      <footer className="border-t border-blue-500/10 py-12 sm:py-16 px-4 sm:px-6 md:px-12 max-w-[1400px] mx-auto">
        <div className="flex flex-col items-center gap-4">
          <p className="text-slate-600 text-xs text-center max-w-xl leading-relaxed">
            ...
            Revenue and valuation figures are estimates for a private company. Not investment advice.
          </p>
